---
title: "MHC1 Analysis in Hawaiian Green Turtles with DADA2- More Conservative Filtering"
...
## Background
This document is similar to the previous document "dada2HIGT_analysis.pdf", but I increased the stringency of the filtering max error rate to be 0.1, AND I filtered out final variants on a per-sample basis to remove variants that represented less than 5% of total reads for that sample. These filtering steps reduced the total number of variants from 112 to 62. 
...
In this case, most sequences had a length of 190bp, which is exactly what I expected given our amplicon length and sequencing platform. Given that there were so few sequences outside of this length, I chose to filter out all of them, to retain just the sequence variants of 190bp lengths.
...
At this point, 34/35 individuals remain. 

#### v. Filter out variants calls in individuals with less than 100 reads AND removing variants with less than 5% of reads in an individual
...
I also filtered out variants on a per-sample basis, removing variants that made up less than 5% of total reads for an individual sample. 


#### vi. Summary statistics

We can see the frequency distribution of variants within this dataset:
